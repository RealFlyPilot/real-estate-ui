...
find . -name "\*.doc.mdx" -exec bash -c 'mv "$1" "${1%.doc.mdx}".mdx' - '{}' \;

find ./src/Components/\*_ -type f -name '_.doc' -exec rename 's/\.doc\$/''/' '{}' \;

➜ find ./src/Components/\*_ -name '_.d.ts' -delete

➜ find ./src/\*_/_.md -type f -empty -print -delete
